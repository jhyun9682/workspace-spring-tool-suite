    ...
    <script type="text/javascript">
    	...
    	$('.btn-account-profile-update ').on('click',function(e){
    		...
    		if($('#account-pass').val()!=$('#account-confirm-pass').val()){
    			...
    			$('#account-confirm-pass').tooltip({'trigger':'hover', 'title':'패쓰워드와 패쓰워드확인이 일치하여야합니다.'});
    			$('#account-confirm-pass').tooltip('show');
    		}else{
    	...
    	$('#form-account-update').on('ropertychange change keyup paste input',function(e){
    		if($('#account-pass').val()!=$('#account-confirm-pass').val()){
    			$('#account-pass').css('border','0.5px dotted orange');
    			$('#account-confirm-pass').css('border','0.5px dotted orange');
    			$('#account-confirm-pass').tooltip({'trigger':'hover', 'title':'패쓰워드와 패쓰워드확인이 일치하여야합니다.'});
    			...
    			$('#account-pass').css('border','');
    			$('#account-confirm-pass').css('border','');
    			$('#account-confirm-pass').tooltip('hide');
    		}	
    	});
